...
class RosObject(ObjectDescription):
    ...
    def handle_signature(self, sig, signode):
        ...
        * it is added to the full name (return value) if not present
        ...
        pkgname = self.options.get("package", self.env.ref_context.get("ros:package"))
        if not obj_type:
    ...
    def add_target_and_index(self, name, sig, signode):
        ...
        if fullname not in self.state.document.ids:
            ...
            if sphinx.version_info[:2] >= (1, 4):
                entry = ("single", indextext, fullname, "", name_to_key(name[0]))
            else:
...
class RosType(RosObject):
    ...
    def handle_signature(self, sig, signode):
        ...
        env_pkg = self.options.get("package", self.env.ref_context.get("ros:package"))
        if not pkg == env_pkg:
    ...
    def get_index_text(self, pkgname, name):
        ...
        newname = name[3]
        if pkgname:
            text = "{} ({} in package {})".format(newname, self.objtype, pkgname)
        else:
    ...
    def add_target_and_index(self, name, sig, signode):
        ...
        if fullname not in self.state.document.ids:
            ...
            indextext = self.get_index_text(pkgname, name)
            if sphinx.version_info[:2] >= (1, 4):
                entry = [("single", indextext, short_name, "", name_to_key(short_name[0]))]
            else:
...
class RosPackageDirective(Directive):
    ...
    optional_arguments = 0
    final_argument_whitespace = False
    option_spec = {"noindex": directives.flag, "deprecated": directives.flag}
    ...
    def run(self):
        ...
        if not noindex:
            ...
            anchor = ros_domain.add_package(pkgname, "deprecated" in self.options)
            targetnode = nodes.target("", "", ids=[anchor])
            self.state.document.note_explicit_target(targetnode)
            ret.append(targetnode)
            indextext = "{} (package)".format(pkgname)
            if sphinx.version_info[:2] >= (1, 4):
                entry = ("single", indextext, anchor, "", name_to_key(pkgname))
            else:
