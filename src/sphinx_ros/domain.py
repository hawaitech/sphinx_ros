...
class RosDomain(Domain):
    ...
    initial_data = {
        ...
        "anonlabels": {"ros-pkgindex": ("ros-pkgindex", ""), "ros-msgindex": ("ros-msgindex", "")},
    }
    indices = [
        RosPackageIndex,
        RosMessageIndex,
    ...
    def resolve_any_xref(self, env, fromdocname, builder, target, node, contnode):
        ...
        for name, obj in matches:
            if obj[1] == "package":
                results.append(
                    ("ros:pkg", self._make_package_refnode(builder, fromdocname, name, contnode))
                )
            else:
                results.append(
                    (
                        "ros:" + self.role_for_objtype(obj[1]),
    ...
    def _make_package_refnode(self, builder, fromdocname, name, contnode):
        ...
        if deprecated:
            title += " (deprecated)"
        return make_refnode(builder, fromdocname, docname, anchor, contnode, title)
